...
#define MANAGED_PTR_SCAN_EVERYWHERE_LOOP(INDX, START, END, SCANINDX, SCANINDX_OFFSET, EXPR) \
   { \
      ScanVar SCANVARNAME(SCANINDX)(END-START+1); \
      ScanVar SCANVARLENGTHNAME(SCANINDX)(1, CARE_SCANVARLENGTHNAME_SPACE); \
      MANAGED_PTR_SCAN_LOOP_INIT(INDX, START, END, SCANVARNAME(SCANINDX), SCANVARLENGTHNAME(SCANINDX), \
                                 SCANINDX_OFFSET, EXPR); \
      int const SCANVARENDNAME(SCANINDX) = END; \
      CARE_CHECKED_MANAGED_PTR_LOOP_START(INDX, START, END, managed_ptr_scaneverywhere_loop_check) { \
         if (INDX == SCANVARENDNAME(SCANINDX)-1) { \
            SCANVARLENGTHNAME(SCANINDX)[0] = SCANVARNAME(SCANINDX)[SCANVARENDNAME(SCANINDX)-START]; \
         } \
         int const SCANINDX = SCANVARNAME(SCANINDX)[INDX-START]; \

#define MANAGED_PTR_SCAN_EVERYWHERE_LOOP_END(END, SCANINDX, SCANLENGTH) \
   } CARE_CHECKED_MANAGED_PTR_LOOP_END(managed_ptr_scaneverywhere_loop_check) \
   SCAN_LOOP_FINAL(END, SCANVARLENGTHNAME(SCANINDX), SCANLENGTH) \
   SCANVARNAME(SCANINDX).free(); \
   SCANVARLENGTHNAME(SCANINDX).free(); \
   }
...
#define MANAGED_PTR_SCAN_EVERYWHERE_LOOP_END(END, SCANINDX, SCANLENGTH) \
   SCAN_EVERYWHERE_LOOP_END(END, SCANINDX, SCANLENGTH)
