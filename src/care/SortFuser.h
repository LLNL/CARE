...
namespace care {
   ...
   void SortFuser<T>::uniq(bool isSorted, bool realloc) {
      ...
      int * pinned_lengths = lengths.getPointer(PINNED, false);
      host_device_ptr<int> * pinned_out_arrays = out_arrays.getPointer(PINNED, false);
      ...
      CARE_LOOP_2D_STREAM_X_LENGTHS(i, 0, maxLength, lengths, a, 0, m_num_arrays, iFlattened)  {
         ...
         pinned_out_arrays[a][i] = result[i+out_offsets[a]];
         if (i == 0) {
            concatenated_lengths[a] = pinned_lengths[a];
         }
      } CARE_LOOP_2D_STREAM_X_LENGTHS_END
