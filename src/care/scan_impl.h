...
void exclusive_scan(chai::ManagedArray<T> data, //!< [in/out] Input data (output also if in place)
   ...
   if (size > 0) {
      ...
      if (size > 1) {
         ...
         const chai::PointerRecord* dataRecord = chai::ArrayManager::getInstance()->getPointerRecord((void *)data.getPointer(care::ExecutionSpace::CPU, false));
         ...
         if (!inPlace) {
            const chai::PointerRecord* outDataRecord = chai::ArrayManager::getInstance()->getPointerRecord((void *)outData.getPointer(care::ExecutionSpace::CPU, false)());
